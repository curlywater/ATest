// 约定：
// title数据类型为String
// userId为主键，数据类型为Number

interface IData {
  userId: number;
  title: string | null;
}
type IDataList = IData[];
type IDataKey = keyof IData;
type WhereParameters = { [name: string]: RegExp };
type OrderMode = "desc" | "asc";
interface CustomArray extends Array<IData> {
  where: (condition: WhereParameters) => CustomArray;
  orderBy: (key: IDataKey, order: OrderMode) => CustomArray;
}

export default function find (origin: IDataList): CustomArray {
  if (!Array.isArray(origin)) {
    throw new Error("parameter should be an array.");
  }
  return Object.defineProperties([...origin], {
    where: {
      value(condition: WhereParameters) {
        const removeIndexes: number[] = [];
        const len = this.length;

        for (let i = 0; i < len; i++) {
          const item = this[i];
          for (let key in condition) {
            if (!condition[key]?.test?.(item[key])) {
              removeIndexes.push(i);
            }
          }
        }

        let rIndex = 0;
        let slowIndex = removeIndexes[rIndex];
        let fastIndex = removeIndexes[rIndex];
        while (fastIndex < len) {
          if (fastIndex === removeIndexes[rIndex]) {
            rIndex++;
          } else {
            this[slowIndex] = this[fastIndex];
            slowIndex++;
          }
          fastIndex++;
        }
        this.length = len - removeIndexes.length;
        return this;
      },
      writable: false,
      enumerable: false,
    },
    orderBy: {
      value(key: IDataKey, order: OrderMode = "desc") {
        const returnValue = order === "desc" ? 1 : -1;
        this.sort((a: IData, b: IData) => {
          const aValue = a[key];
          const bValue = b[key];
          if (aValue === null) {
            return bValue === null ? 0 : returnValue;
          } else if (bValue === null) {
            return aValue === null ? 0 : -returnValue;
          } else {
            return aValue > bValue ? -returnValue : returnValue;
          }
        });
        return this;
      },
      writable: false,
      enumerable: false,
    },
  });
};
// 查找 data 中，符合条件的数据，并进⾏排序
